...
def create
  ...
  if !::Milia.use_recaptcha  ||  verify_recaptcha
    ...
    Tenant.transaction  do
      ...
      if @tenant.errors.empty?   # tenant created
        if @tenant.plan = 'premium'
          @payment = Payment.new({email: user_params["email"],
            ...
            tenant: @tenant })
          flash[:error] = "Please check registrations erros" unless @payment.valid?
      ...
      if flash[:error].blank? || falsh[:error].empty?
        ...
        resource.valid?
        log_action("Payment proccesing failed", @tenant)
        render :new and return
      end# if then else no tenant erros
