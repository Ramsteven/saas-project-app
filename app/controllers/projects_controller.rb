class ProjectsController < ApplicationController
  before_action :set_project, only: %i[ show edit update destroy user add_user ]
  ...
  def users
    ...
    @other_users = @tenant_users.where(tenant_id: @tenant.id, is_admin: false) - (@project_users + [current_user]) 
  end
  ...
  private
    ...
    def verify_tenant
      unless params[:tenant_id] == Tenant.current_tenant_id.to_s
        redirect_to :root, 
                          flash: { error: 'You are not authorized to access any orginazation other that your own' }
      end
    end
end
